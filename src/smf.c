...
__unused static bool smf_execute_ancestor_entry_actions(struct smf_ctx* const ctx, const struct smf_state* target)
{
    ...
    for (const struct smf_state* to_execute = get_last_of(target); to_execute != NULL && to_execute != target;
...
__unused static bool smf_execute_ancestor_run_actions(struct smf_ctx* ctx)
{
    ...
    for (const struct smf_state* tmp_state = ctx->current->parent; tmp_state != NULL; tmp_state = tmp_state->parent) {
...
void smf_set_state(struct smf_ctx* const ctx, const struct smf_state* target)
{
     ...
     */
    if (internal->exit) {
        ESP_LOGW(TAG, "Calling %s from exit action", __func__);
        return;
    }
